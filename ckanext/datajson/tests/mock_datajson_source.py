...
class MockDataJSONHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path == 'https://www.archive.arm.gov/metadata/data.json':
            self.sample_datajson_file = 'arm.data.json'
            self.test_name = 'arm'
        elif self.path == 'http://www.usda.gov/data.json':
            self.sample_datajson_file = 'usda.gov.data.json'
            self.test_name = 'usda'
        elif self.path == 'http://some404/data.json':
            self.test_name = 'e404'
            self.respond('Not found', status=404)
        elif self.path == 'http://some500/data.json':
            self.test_name = 'e500'
